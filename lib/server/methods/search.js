...
Meteor.methods({
  TMDBSearch:function(search){
    ...
    try {
      var movie_response = HTTP.call("GET", "https://api.themoviedb.org/3/search/movie?api_key=" + TMDBSearch.api + "&language=" + TMDBSearch.language + "&query=" + search, {});
      var tv_response = HTTP.call("GET", "https://api.themoviedb.org/3/search/tv?api_key=" + TMDBSearch.api + "&language=" + TMDBSearch.language + "&query=" + search, {});
    }
    catch (error) {
      console.log(error);
